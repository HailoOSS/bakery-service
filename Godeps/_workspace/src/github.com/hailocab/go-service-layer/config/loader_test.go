...
func TestDefaultLoaderKilledWhenReplaced(t *testing.T) {
	...
	// We need to setup the H2_CONFIG_SERVICE_ADDR and EC2_REGION environment variables. We expect that the service load
	// will fail anyway (there is no "testservice" and the test will not be running in a real environment), but it
	// should still construct the loader
	oldAddr := os.Getenv("H2_CONFIG_SERVICE_ADDR")
	oldRegion := os.Getenv("EC2_REGION")
	os.Setenv("H2_CONFIG_SERVICE_ADDR", "test_config_addr")
	os.Setenv("EC2_REGION", "test_ec2_region")
	defer func() {
		os.Setenv("H2_CONFIG_SERVICE_ADDR", oldAddr)
		os.Setenv("EC2_REGION", oldRegion)
	}()

	err = LoadFromService("testservice")
	assert.NoError(t, err)
